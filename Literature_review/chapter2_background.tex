...
The Lorenz-1960 system consists of three coupled nonlinear ODEs for meteorological modeling. We target the formulation from \cite{matthews2024pinnde} (Section 4.2):
...
\textbf{Euler's method} approximates the solution via a first-order Taylor expansion:
\begin{equation}
y_{n+1} = y_n + h \cdot f(t_n, y_n)
\end{equation}
where $h$ is the step size. It is simple but accumulates errors quickly.

The \textbf{fourth-order Runge-Kutta method (RK4)} evaluates the derivative at multiple points per step for higher accuracy:
\begin{equation}
...
where $\mathbf{w}^{(i)}$ are weights, $b^{(i)}$ is bias, and $\sigma$ is an activation function \cite{lau2020oden}. The \textbf{Universal Approximation Theorem} \cite{lau2020oden, babni2025error} guarantees that a network with at least one hidden layer can approximate any continuous function to arbitrary accuracy. Common activation functions include:
\begin{itemize}
...
Instead of stepping through the domain, ANNs learn a continuous solution function over the entire domain. The standard approach \cite{mall2013comparison, okereke2021novel} constructs a trial solution:
...
PINNs embed governing equations directly into the loss function \cite{matthews2024pinnde, babni2025error, audu2026neural}:
...
\begin{figure}[ht]
    \centering
    % Please ensure the image file is named 'ann_architecture.png' and placed in the same directory as the tex files, or update the path below.
    \includegraphics[width=0.8\textwidth]{ann_architecture.png}
...
\begin{figure}[ht]
    \centering
    % Please ensure the image file is named 'pinn_architecture.png' and placed in the same directory as the tex files, or update the path below.
    \includegraphics[width=0.9\textwidth]{pinn_architecture.png}
...
\begin{table}[!ht] % Using !ht is safer and won't crash your document
    ...
    \begin{tabularx}{\textwidth}{| 
        ...
        \textbf{Our Work} & \textbf{Plain ANN: Traditional feedforward network (No PINN).} & \textbf{Direct solution of coupled ODE systems.} & \textbf{Comprehensive study of hidden layers and density.} & \textbf{Primary test case for predicting chaotic dynamics.} & \textbf{Purely data-driven ANN without physics aids.} \\
        \hline
...
This chapter established the necessary background for our research. We reviewed the mathematical foundations of ODEs, the Lorenz-1960 system, classical numerical methods, and ANN-based solving approaches. The literature review covered 13 research papers spanning physics-informed methods, pure ANN approaches, architecture studies, Lorenz system solvers, Neural ODEs, and hybrid techniques.
