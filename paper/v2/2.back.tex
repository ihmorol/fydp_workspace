...
\begin{tikzpicture}[
    font=\footnotesize,
    neuron/.style={circle, draw=black!55, fill=teal!16, minimum size=7mm, inner sep=0pt},
    inn/.style={circle, draw=black!55, fill=orange!22, minimum size=8mm, inner sep=0pt},
    outn/.style={circle, draw=black!55, fill=blue!16, minimum size=8mm, inner sep=0pt},
    conn/.style={draw=black!22, line width=0.4pt}
]
% input
\node[inn] (i1) at (0,0) {$t$};
% hidden layer A
\foreach \i/\y in {1/2.1,2/0.9,3/-0.9,4/-2.1}{\node[neuron] (a\i) at (2,\y) {};}
\node at (2,0) {$\vdots$};
% hidden layer B
\foreach \i/\y in {1/2.1,2/0.9,3/-0.9,4/-2.1}{\node[neuron] (b\i) at (4,\y) {};}
\node at (4,0) {$\vdots$};
% output
\node[outn] (o1) at (6, 1.2) {$x$};
\node[outn] (o2) at (6, 0.0) {$y$};
\node[outn] (o3) at (6,-1.2) {$z$};
% connections
...
% layer captions
\node[align=center] at (0,-3.0) {Input\\time $t$};
\node[align=center] at (3,-3.0) {Hidden layers\\(depth \& width varied)};
\node[align=center] at (6,-3.0) {Output\\state $(x,y,z)$};
\end{tikzpicture}
\caption{Feedforward ANN used in this study. A single scalar input, time $t$, is propagated through a stack of hidden layers to predict the Lorenz-1960 state $(x(t),y(t),z(t))$. The number of hidden layers, the neurons per layer, and the activation function are the axes compared in this work \cite{Bre}.}
\label{fig:ann_arch}
\end{figure}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    font=\footnotesize, node distance=0.8cm and 1.0cm,
    block/.style={rounded corners=3pt, draw=black!55, fill=black!4, line width=0.7pt,
                  align=center, inner sep=6pt, minimum height=1.2cm},
    nn/.style={rounded corners=3pt, draw=teal!60, fill=teal!12, line width=0.8pt,
               align=center, inner sep=6pt, minimum height=1.2cm, text width=2.3cm},
    res/.style={rounded corners=3pt, draw=blue!55, fill=blue!8, line width=0.8pt,
                align=left, inner sep=7pt},
    loss/.style={rounded corners=3pt, draw=red!55, fill=red!8, line width=0.8pt,
                 align=center, inner sep=6pt, minimum height=1.0cm},
    flow/.style={-{Latex[length=2.6mm]}, line width=0.8pt, draw=black!65}
]
\node[block] (t) {$t$};
\node[nn, right=of t] (net) {Feedforward NN\\$N(t;\theta)$};
\node[block, right=of net] (out) {$\tilde{x},\ \tilde{y},\ \tilde{z}$};
\node[block, right=1.2cm of out] (ad) {Automatic\\differentiation\\$\dot{\tilde{x}},\dot{\tilde{y}},\dot{\tilde{z}}$};
...
\node[block, below=1.1cm of lphys, fill=orange!12, draw=orange!70] (total)
...
\caption{Physics-Informed Neural Network for the Lorenz-1960 system. The network outputs $(\tilde{x},\tilde{y},\tilde{z})$ are differentiated to form the ODE residuals $r_x, r_y, r_z$, whose mean square becomes a physics loss added to the data loss \cite{Zhai}. The plain-ANN approach adopted in this report deliberately omits the physics term ($\lambda=0$), training on the data loss alone.}
\label{fig:pinn_arch}
\end{figure}
...
\begin{table}[!ht] % Using !ht is safer and won't crash your document
    ...
    \begin{tabularx}{\textwidth}{| 
        ...
        PinnDE \cite{matthews2024pinnde} & PINN + DeepONet: Integrates physics laws and operator networks into the loss function. & Solves both ODEs and PDEs using governing physics. & No systematic study of network structural impact. & Evaluated specifically as a complex dynamical system. & Uses physics-informed logic, not a plain ANN. \\
        \hline
        
        ANN Survey \cite{pratama2022ann} & PyDEns / ANN: Standard networks for field estimation. & Primary focus on solving Partial Differential Equations (PDEs). & General overview without specific layer/neuron testing. & The study does not address the Lorenz model. & Covers standard ANN architectures for PDEs. \\
        \hline
        
        Neural ODEs \cite{chen2018neural} & Neural ODE: Derivative represented as a neural network. & Models continuous-time dynamics for ODEs. & Focuses on functional flexibility, not structural depth. & Not specifically tested on the Lorenz system. & Relies on specialized ODE-solver integration. \\
        \hline
        
        Aslam et al. \cite{aslam2024neuro} & ANN + PSO-NNA: ANN with Particle Swarm Optimization. & Applied to solving Ordinary Differential Equations. & Optimization focuses on weights, not architectural depth. & Directly applied to Lorenz-based chaotic dynamics. & Uses heuristic optimization instead of plain training. \\
        \hline
        
        ODEN \cite{lau2020oden} & Plain ANN: Standard feedforward network approach. & General application for approximating ODE solutions. & Provides a limited (partial) study on hidden layers. & Tested on other benchmarks, not Lorenz-1960. & Utilizes a standard, plain ANN configuration. \\
        \hline
        
        Mall \& Chakraverty \cite{mall2013comparison} & ANN: Implementation of neural networks for equations. & Targets solutions for ODEs in engineering. & Includes a detailed study on network architecture. & Application restricted to simpler, non-chaotic ODEs. & Employs a basic feedforward ANN structure. \\
        \hline
        
        Xiong \cite{xiong2022new} & ANN: Simple neural solver for dynamic systems. & Focused on solving Ordinary Differential Equations. & Does not explore the impact of different layer counts. & Does not address the chaotic Lorenz-1960 model. & Uses a traditional, data-driven ANN model. \\
        \hline
        
        Babni et al. \cite{babni2025error} & PINN: Physics-Informed Neural Networks approach. & Solves ODEs by embedding equations in the loss function. & No investigation into depth or width performance. & Not applied to the meteorological Lorenz model. & Strictly uses physics-informed constraints. \\
        \hline
        
        Dubey et al. \cite{dubey2024solving} & Plain ANN: Feedforward network trained on numerical data. & Applied to various Ordinary Differential Equations. & Lacks a systematic study of network architecture. & No specific testing on the Lorenz system. & Uses a standard numerical data training pipeline. \\
        \hline
        
        Okereke et al. \cite{okereke2021novel} & ANN: Basic neural network models for ODEs. & Aimed at solving standard ODE systems. & Does not perform structural performance analysis. & No mention or testing of the Lorenz-1960 system. & Follows a traditional, plain ANN approach. \\
        \hline
        
        Audu et al. (2024) \cite{audu2024utilizing} & Plain ANN: Usage for initial value problems in ODEs. & Specifically handles ODE solution approximation. & No detailed evaluation of layers or neurons. & Focused on other common types of ODEs. & Relies on a standard feedforward structure. \\
        \hline
        
        Audu et al. (2026) \cite{audu2026neural} & PINN vs ANN: A comparative study of two methods. & Analysis restricted to Ordinary Differential Equations. & Lacks an in-depth architecture comparison study. & Not applied to the Lorenz-1960 model. & Discusses ANN only in comparison to PINN. \\
        \hline
        
        Murugesh et al. \cite{murugesh2025novel} & Hybrid: Combines different neural modeling techniques. & Solves ODEs using integrated hybrid approaches. & Architecture optimization is not the primary focus. & Does not utilize the Lorenz-1960 test case. & Incorporates external optimization methods. \\
        \hline

        Lagaris et al. \cite{lagaris1998ann} & ANN: Trial-solution method using a feedforward network. & Solves ODEs, ODE systems, and PDEs. & No systematic study of layers or neurons. & Does not test the Lorenz-1960 system. & Plain feedforward ANN trained on the ODE residual. \\
        \hline

        Raissi et al. \cite{raissi2019pinn} & PINN: Embeds governing equations in the loss function. & Targets forward and inverse nonlinear PDEs. & No systematic study of depth or width. & Not applied to the Lorenz-1960 model. & Uses physics-informed constraints, not a plain ANN. \\
        \hline

        Lorenz \cite{lorenz1960max} & Analytical: Reduces the vorticity equation to three ODEs. & Derives the coupled nonlinear Lorenz-1960 ODEs. & Predates neural network architecture studies. & Defines the original Lorenz-1960 system. & Uses no neural network of any kind. \\
        \hline

        Hornik et al. \cite{hornik1989multilayer} & Theory: Proves the universal approximation property. & Not concerned with differential equations. & Shows one hidden layer suffices for approximation. & Does not address the Lorenz-1960 system. & Covers feedforward networks in general, not a solver. \\
        \hline
